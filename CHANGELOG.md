...
- **MCP 架构优化**：MCP 服务器现在复用已运行的 gateway daemon，而非每次启动独立进程
  - 首次调用时自动启动 daemon（如果未运行）
  - 后续调用通过 HTTP 与 daemon 通信，响应更快
  - 与 CLI 命令保持一致的架构
...
- **Python 版本锁定**：将 Python 最低版本要求提升至 3.13，同步更新 ruff 和 mypy 配置
